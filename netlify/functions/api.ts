...
export default async (request: Request, _context: Context) => {
  ...
  if (!supabase) return json({ error: "Database API is not configured." }, 503);
  const pathname = new URL(request.url).pathname.replace(/\/+$/, "") || "/";
  try {
    if (pathname.startsWith("/api/admin")) return await adminRoute(request, pathname, supabase);
    if (request.method === "GET" && pathname === "/api/config") {
      return json({ metaPixelId: Netlify.env.get("META_PIXEL_ID") || "", timezone: TIMEZONE });
    }
    if (request.method === "GET" && pathname === "/api/cars") return await publicCars(request, supabase);
    if (request.method === "GET" && pathname === "/api/filters") return await filters(supabase);
    const slotMatch = pathname.match(/^\/api\/cars\/([^/]+)\/slots$/);
    if (request.method === "GET" && slotMatch) return await slots(decodeURIComponent(slotMatch[1]), supabase);
    const carMatch = pathname.match(/^\/api\/cars\/([^/]+)$/);
    if (request.method === "GET" && carMatch) return await carDetail(decodeURIComponent(carMatch[1]), supabase);
    if (request.method === "POST" && pathname === "/api/leads") return await submitLead(request, supabase);
    return json({ error: "Not found." }, 404);
  } catch (error) {
