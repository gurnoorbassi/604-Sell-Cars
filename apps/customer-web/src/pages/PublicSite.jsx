        ...
        <div className="motion-section mx-auto w-[min(1340px,92vw)]">
          ...
          <div className="mt-9 grid grid-cols-2 gap-3 md:grid-cols-5">
            {bodyTypes.map(([label, matcher]) => {
              ...
              return (
                <a key={label} href={`${WEBSITE_URL}/inventory?${label === "EV" ? "fuel=Electric" : `bodyType=${encodeURIComponent(label)}`}`} className="group relative aspect-[4/3] overflow-hidden border border-white/10 bg-[radial-gradient(circle_at_center,#20242a_0%,#0b0d10_78%)]">
                  {image && <VehicleImage sources={carImages(car)} alt="" className="h-full w-full object-cover object-center" />}
