    ...
    <div className="min-h-screen bg-neutral-100 text-neutral-950">
      ...
      <main className="mx-auto w-[min(1400px,94vw)] py-10">
        <div className="flex flex-wrap items-end justify-between gap-5">
          ...
          <div className="flex gap-2 rounded bg-neutral-900 p-1 text-white">
            <button onClick={() => setTab("leads")} className={`px-4 py-2 ${tab === "leads" ? "bg-white text-black" : ""}`}>Leads</button>
            <button onClick={() => setTab("inventory")} className={`px-4 py-2 ${tab === "inventory" ? "bg-white text-black" : ""}`}>Inventory</button>
          </div>
        </div>
        ...
        {tab === "leads" ? (
          ...
          <>
            <div className="mt-8 flex justify-end"><button onClick={() => editCar()} className="bg-red-600 px-5 py-3 font-black text-white">+ Add vehicle</button></div>
            {formOpen && (
            ...
            <div className="mt-5 overflow-auto border border-neutral-200 bg-white">
              <table className="w-full min-w-[800px] border-collapse text-left"><thead><tr className="border-b bg-neutral-50 text-xs uppercase text-neutral-500"><th className="p-3">Vehicle</th><th>Price</th><th>Mileage</th><th>Lot</th><th>Status</th><th>Media</th><th /></tr></thead>
                <tbody>{cars.map((car) => <tr key={car.id} className="border-b"><td className="p-3"><strong>{carName(car)}</strong><small className="block text-neutral-500">{car.stock}</small></td><td>${Number(car.price_amount || 0).toLocaleString()}</td><td>{Number(car.mileage || 0).toLocaleString()} km</td><td className={car.lot === "LOCATION_REQUIRED" ? "font-bold text-red-600" : ""}>{car.lot_name}<small className="block">{car.lot_address}</small></td><td>{car.status}</td><td>{(car.images || []).length} photos · {(car.videos || []).length} videos</td><td><button onClick={() => editCar(car)} className="p-3 font-bold text-red-600">Edit</button></td></tr>)}</tbody>
              </table>
            </div>
          </>
        )}
